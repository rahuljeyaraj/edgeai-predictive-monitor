...
length. The expected length itself is the caller's node's own committed
`entry.input_dim` (pipeline/manager.py's _infer_sensor_config_and_dim) --
not every node necessarily uses the same per-channel bin count.
"""
...
def build_feature_vector(frame: SensorFrame, sensor_config: FrozenSet[SensorChannel],
    ...
    Raises if the frame's bin counts don't match -- that's a base
    station/node misconfiguration, not a normal runtime case to silently
    paper over.
    """
    vector: Tuple[float, ...] = ()
