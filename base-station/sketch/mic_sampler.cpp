...
/* Raw, un-FFT'd window. Originally only published in FUSER_RAW_CAPTURE_MODE
 * (offline experimentation); normal mode now also needs this unconditionally
 * for the mic decimated time-domain section (docs/CHART_CLUTTER_PLAN.md S1),
 * so this is no longer gated. Published as float32 (matching the TIME_SERIES
...
/* Raw window access - the window length is mic_fft_size() (MIC_FFT_LEN),
 * the same accessor the normal spectrum path already uses; no separate raw
 * sample-count accessor needed. */
