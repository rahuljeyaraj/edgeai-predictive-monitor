...
print("epoch,http,ms,worst_age,offline,frames_ok,arm_err,onframe_err")
end = time.time() + DUR
while time.time() < end:
    tick = time.time()
    try:
        ...
        now = time.time()
        ages = [now - v["last_seen"] for v in nodes.values()
                if v.get("last_seen") is not None]
        worst = max(ages) if ages else -1
        offline = sum(1 for a in ages if a > OFFLINE_AFTER_S)
    ...
    try:
        ...
        fok = aerr = oerr = ""
    print(f"{tick:.0f},{code},{ms:.0f},{worst:.1f},{offline},{fok},{aerr},{oerr}", flush=True)
    time.sleep(max(0, 1 - (time.time() - tick)))
